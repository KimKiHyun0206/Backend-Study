...
요청한 리소스의 URI가 변경되었음을 의미한다. 새로운 URI가 응답해서 주어질 수도 있다

### 302 Found

* 요청한 리소스의 URI가 일시적으로 변경되었음을 의미한다
* 새롭게 변경된 URI는 나중에 만들어질 수도 있다
* 클라이언트는 향후의 요청도 반드시 동일한 URI로 해야 한다
...
* 만약 첫 번째 요청이 POST 였다면 두 번째 요청도 POST여야 한다

### 308 Permanent Redirect

* 리소스가 HTTP 응답 헤더의 `Location:` 에 명시된 영구히 다른 URI에 위치함을 알린다
...
* 이것은 서버에서 요청 URI와 연결된 스킴과 권한을 구성하여 응답을 생성할 수 없을 때 보내진다
### 423 Locked
리소스에 접근하는 것이 잠겨있따
